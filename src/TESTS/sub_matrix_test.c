...
START_TEST(sub_matrix_4) {
  ...
  ck_assert_int_eq(sub_matrix(&A, &B, &result), 0);
  answer.matrix[0][0] = -360, answer.matrix[0][1] = 0, answer.matrix[0][2] = 0;
  ck_assert_int_eq(eq_matrix(&result, &answer), 1);
  remove_matrix(&A);
  remove_matrix(&B);
  remove_matrix(&answer);
...
START_TEST(sub_matrix_6) {
  ...
  A.matrix[0][0] = 11.11, A.matrix[0][1] = -15.47;
  A.matrix[1][0] = 28.64, A.matrix[1][1] = -34.48;
  B.matrix[0][0] = 10.01, B.matrix[0][1] = -6.17;
  B.matrix[1][0] = 1.34, B.matrix[1][1] = 5.52;
  ck_assert_int_eq(sub_matrix(&A, &B, &result), 0);
  answer.matrix[0][0] = 1.1, answer.matrix[0][1] = -9.3;
  answer.matrix[1][0] = 27.3, answer.matrix[1][1] = -40;
  ck_assert_int_eq(eq_matrix(&result, &answer), 1);
  remove_matrix(&A);
  remove_matrix(&B);
  remove_matrix(&answer);
...
TCase *tcase_sub_matrix(void) {
  TCase *tcase = tcase_create("sub_matrix");

  tcase_add_test(tcase, sub_matrix_0);
  tcase_add_test(tcase, sub_matrix_1);
  tcase_add_test(tcase, sub_matrix_2);
  tcase_add_test(tcase, sub_matrix_3);
  tcase_add_test(tcase, sub_matrix_4);
  tcase_add_test(tcase, sub_matrix_5);
  tcase_add_test(tcase, sub_matrix_6);
  tcase_add_test(tcase, sub_matrix_7);
  tcase_add_test(tcase, sub_matrix_8);
  return tcase;
}
